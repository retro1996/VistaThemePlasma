...
void AppletsLayout::setContainment(Plasma::Containment *containment)
{
    ...
    QList<Plasma::Applet* > containmentPlasmoids = m_containment->applets();

    QStringList dirs = QStandardPaths::locateAll(QStandardPaths::GenericDataLocation, QStringLiteral("plasma/plasmoids/io.gitgud.catpswin56.sidebar"), QStandardPaths::LocateDirectory);

    // qDebug() << dirs.first();
    if(containmentPlasmoids.size() == 0)
        m_containment->createApplet(dirs.first());
    else {
        for(int i = 0; i < containmentPlasmoids.size(); i++) {
            if(containmentPlasmoids.at(i)->pluginName() != QStringLiteral("io.gitgud.catpswin56.sidebar")) {
                if(i == (containmentPlasmoids.size() - 1))
                    m_containment->createApplet(dirs.first());
            }
        }
    }
