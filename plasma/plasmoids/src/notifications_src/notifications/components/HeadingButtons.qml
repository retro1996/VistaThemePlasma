...
RowLayout {
    ...
    Kirigami.Heading {
        ...
        function generateAgoText() {
            ...
            if (deltaMinutes < 1) {
                ...
                return headingButtons.modelInterface.inHistory
                    ? i18ndc("plasma_applet_io.gitgud.catpswin56.notifications", "Notification was added less than a minute ago, keep short", "Just now")
                    : "";
            }

            // Received less than an hour ago, show relative minutes
            if (deltaMinutes < 60) {
                return i18ndcp("plasma_applet_io.gitgud.catpswin56.notifications", "Notification was added minutes ago, keep short", "%1 min ago", "%1 min ago", deltaMinutes);
            }
        ...
        function generateRemainingText() {
            ...
            if (eta < 60) { // 1 minute
                return i18ndcp("plasma_applet_io.gitgud.catpswin56.notifications", "seconds remaining, keep short",
                              "%1 s remaining", "%1 s remaining", Math.round(eta));
            }
            if (eta < 60 * 60) {// 1 hour
                return i18ndcp("plasma_applet_io.gitgud.catpswin56.notifications", "minutes remaining, keep short",
                              "%1 min remaining", "%1 min remaining",
                              Math.round(eta / 60));
            }
            if (eta < 60 * 60 * 5) { // 5 hours max, if it takes even longer there's no real point in showing that
                return i18ndcp("plasma_applet_io.gitgud.catpswin56.notifications", "hours remaining, keep short",
    ...
    ToolButton {
        ...
        property string text: headingButtons.modelInterface.configureActionLabel || i18nd("plasma_applet_io.gitgud.catpswin56.notifications", "Configure")
    ...
    ToolButton {
        ...
        property string text: headingButtons.modelInterface.dismissed
            ? i18ndc("plasma_applet_io.gitgud.catpswin56.notifications", "Opposite of minimize", "Restore")
            : i18nd("plasma_applet_io.gitgud.catpswin56.notifications", "Minimize")
        Accessible.description: headingButtons.__applicationName
    ...
    ToolButton {
        ...
        PlasmaCore.ToolTipArea {
            anchors.fill: parent
            mainText: i18nd("plasma_applet_io.gitgud.catpswin56.notifications", "Close")
            location: PlasmaCore.Types.Floating | PlasmaCore.Types.Desktop
        }
    }
}
